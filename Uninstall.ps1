...
function Read-HostLock {try{return Get-Content (Join-Path $State 'host.lock.json') -Raw|ConvertFrom-Json}catch{return $null}}
...
function Lock-MatchesLiveHost {
  $lock=Read-HostLock;if(-not $lock -or -not $lock.pid){return $false};$processId=[int]$lock.pid
  try{$null=Get-Process -Id $processId -ErrorAction Stop}catch{return $false}
  if(-not $lock.processIdentity){return $true}
  $identity=[string]$lock.processIdentity;if($identity -notmatch '^win32:\d+$'){return $true}
  return (Test-ExactProcessAlive $processId $identity)
}
function Get-VerifiedOwner($cfg,$h){
  if(-not $h -or [string]$h.version -ne [string]$cfg.programVersion){Fail 'Authenticated Host release identity is missing or stale. Run Setup/Repair first; no files were removed.'}
  if(-not $h.host -or -not $h.host.pid -or -not $h.host.processIdentity -or [string]$h.host.instanceId -notmatch '^[a-f0-9]{32}$'){Fail 'Authenticated Host process identity is incomplete. Run Setup/Repair first; no files were removed.'}
  ...
  $lock=Read-HostLock
  if(-not $lock -or [int]$lock.pid -ne $processId -or [string]$lock.processIdentity -ne $identity -or [string]$lock.instanceId -ne [string]$h.host.instanceId){Fail 'Authenticated Host health/lock ownership does not match; no files were removed.'}
  return [pscustomobject]@{pid=$processId;processIdentity=$identity;instanceId=[string]$h.host.instanceId}
}
