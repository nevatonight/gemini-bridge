...
import { antigravityAuthStatus } from './auth-state.mjs';
...
else if(cmd==='auth-status'){const cfg=await ensureRuntimeConfig(root);const status=await antigravityAuthStatus(root,cfg);console.log(JSON.stringify({present:status.authenticated===true,authenticated:status.authenticated,error:status.error||null,provider:'antigravity'}));}
