...
import { hasGeminiCredentials } from './auth-state.mjs';
...
const server=http.createServer({maxHeaderSize:16*1024},async(req,res)=>{
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/v1/health'){const health=await core.health();const credentialsPresent=await hasGeminiCredentials(root).catch(()=>false);return json(res,200,{...health,auth:{credentialsPresent},host:{instanceId:hostInstanceId,pid:process.pid,processIdentity:hostProcessIdentity,launchNonce}});}
