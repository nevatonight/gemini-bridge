# Gemini Bridge 0.4.7 — Complete Project Documentation

> This document is the single detailed technical and operational reference for the Gemini Bridge 0.4.7 release tree. `README.md` is intentionally shorter. When implementation and documentation disagree, the physically shipped code and release tests are authoritative; update this document as part of the same change before release.

## 1. Product purpose

Gemini Bridge is a local Windows companion that lets a user work with **Gemini CLI / Google AI Pro alongside ChatGPT** without pretending that Gemini is an OpenAI-hosted model or giving Gemini direct control over ChatGPT.
...
The product is deliberately local-first. The Bridge Host, SQLite history, snapshots, pairing token, managed Gemini CLI runtime and isolated Gemini profile are stored on the user's computer. Gemini requests leave the computer through the pinned Gemini CLI. ChatGPT and OpenAI credentials are not used by the Bridge.
...
The release pins an **app-local Gemini CLI 0.55.1**.

The user's global Gemini CLI is neither required nor modified. Setup installs/reuses a versioned managed runtime below the Gemini Bridge local-data directory. A candidate runtime is accepted only when:

- package metadata is valid;
- `bin.gemini` resolves to a safe file inside the managed npm package;
- the package version is the pinned version;
- a bounded real `--version` probe matches the pinned version.

The Host repeats runtime verification and fails closed on version drift.
...
2. Run `Setup.cmd`.
3. Setup displays six high-level stages instead of appearing frozen during npm/runtime work.
...
6. The managed Gemini runtime is verified or installed.
7. The new Host is started and verified as the **exact instance started by this Setup transaction**.
8. If Google authentication is missing, Gemini sign-in is completed before the Dashboard opens.
9. Desktop/Startup shortcuts have already been installed after the exact candidate Host passed verification.
10. The Dashboard opens only when isolated Google credentials are present; if sign-in is cancelled, Setup leaves the verified installation installed and tells the user how to complete sign-in later.

If Google sign-in is cancelled, the already-healthy program installation is retained. Credential presence is not described as proof that an access token is unexpired; real provider validity is proven only by an authenticated Gemini request. Setup explains that authentication is incomplete and points the user to the **Gemini Bridge Sign-in** shortcut.
...
`User prompt → local UI/extension → authenticated loopback Host → Bridge Core → Gemini CLI → Google → streamed result → SQLite history → UI`
...
- `runtime\` — versioned app-local Gemini CLI npm runtimes;
- `gemini-home\` — isolated Gemini CLI user/profile data used only by Gemini Bridge;
...
- **Gemini Bridge Sign-in** — runs/refreshes Google authentication for the isolated Bridge Gemini profile;
...
- verify or install the pinned app-local Gemini CLI;
...
- optionally remove history, Google Bridge profile, backups and managed Gemini runtime.
...
Runs the managed Gemini CLI in the isolated Bridge profile. It uses the Bridge-owned empty auth working directory and explicit trust bypass for that app-owned directory. Authentication success is followed by a local credential-presence check.

Authentication writes/uses the isolated Gemini profile. Setup/Auth verify credential-file presence after the interactive flow, but **Host health remains a runtime/version health check and does not make Host health an OAuth-validity oracle**. Token expiry/refresh/account acceptance is verified only when Gemini actually contacts the provider.
...
Gemini Bridge uses its own `gemini-home`; it does not reuse the user's global Gemini CLI profile unless data is explicitly placed there by Bridge authentication.
...
- The authoritative tool boundary is the generated `tools.core` allowlist plus the isolated Gemini home/working directory; shell is absent from that allowlist. Bridge also requests disabled MCP/extensions/skills/hooks behavior as defense-in-depth, but the security argument does not depend on a single file-based `admin` setting being authoritative.
...
9. Verify/reuse/install the pinned managed Gemini runtime.
...
The program may still be installed and healthy. Run the Desktop **Gemini Bridge Sign-in** shortcut and complete authentication before sending Gemini requests.
...
5. Real Ask through Gemini CLI 0.55.1.
...
- Agent does not execute shell/tests in 0.4.7; edits may require the user/another tool to run project-specific verification afterwards.
...
- real Gemini CLI 0.55.1 + Google OAuth;
...
**Managed Gemini runtime** — app-local pinned npm installation of Gemini CLI owned by Gemini Bridge, separate from the user's global CLI.

**Gemini home** — isolated Gemini CLI profile/OAuth directory used by Bridge.
