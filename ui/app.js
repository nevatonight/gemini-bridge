(()=>{
  ...
  const I18N={
    ru:{
      connecting:'Подключение…',technicalInfo:'Техническая информация',project:'Проект',projectSettings:'Настройки проекта',conversations:'Диалоги',newThread:'Новый диалог',pendingWork:'Неприменённые изменения',authChecking:'Проверяем вход…',googleSignIn:'Войти снова',copyContext:'Скопировать контекст проекта',addChatgptPanel:'Добавить панель в ChatGPT',
      ...
      starterProjectName:'Мой проект',newThreadTitle:'Новый диалог',workspaceNotSet:'Папка не выбрана',workspaceSet:'Папка: {path}',bridgeWorking:'Gemini Bridge работает',bridgeUnavailable:'Gemini Bridge недоступен',authPresent:'Данные входа найдены',authMissing:'Требуется вход в Google',authUnknown:'Статус входа неизвестен',
      runQueued:'Запуск…',runRunning:'Gemini отвечает…',runCompleted:'Готово',runWaitingApply:'Изменения готовы к просмотру',runRecovery:'Доступно восстановление',runApplying:'Безопасно применяем…',runApplied:'Применено',runConflict:'Обнаружены внешние изменения',runApplyRecovery:'Нужно восстановление применения',runDiscarded:'Отменено',runFailed:'Ошибка',runCancelled:'Отменено пользователем',
      ...
      failedTitle:'Не удалось получить ответ Gemini',cancelledTitle:'Запрос отменён',retry:'Повторить',technicalDetails:'Технические подробности',googleSignInHelp:'Помощь со входом Google',
      ...
      authHelpTitle:'Вход в Google Gemini',authHelpBody:'На рабочем столе запусти ярлык «Gemini Bridge Sign-in». Войди в аккаунт Google с доступом к Gemini, дождись сообщения об успешном входе и затем повтори запрос здесь.',webHelpTitle:'Панель Gemini внутри ChatGPT',webHelpBody:'Chrome/Edge → Расширения → Режим разработчика → «Загрузить распакованное расширение» → выбери папку %LOCALAPPDATA%\\GeminiBridge\\web-extension. После Setup/Repair нажми «Обновить» у расширения.',
      noProjectName:'Введи название проекта.'
    },
    en:{
      connecting:'Connecting…',technicalInfo:'Technical information',project:'Project',projectSettings:'Project settings',conversations:'Conversations',newThread:'New thread',pendingWork:'Pending Agent work',authChecking:'Checking sign-in…',googleSignIn:'Sign in again',copyContext:'Copy project context',addChatgptPanel:'Add panel to ChatGPT',
      ...
      failedTitle:'Could not get a Gemini response',cancelledTitle:'Request cancelled',retry:'Retry',technicalDetails:'Technical details',googleSignInHelp:'Google sign-in help',
      ...
      authHelpTitle:'Google Gemini sign-in',authHelpBody:'Run the “Gemini Bridge Sign-in” shortcut on the Desktop. Sign in with the Google account that can use Gemini, wait for the success message, then retry the request here.',webHelpTitle:'Gemini panel inside ChatGPT',webHelpBody:'Chrome/Edge → Extensions → Developer mode → Load unpacked → choose %LOCALAPPDATA%\\GeminiBridge\\web-extension. After Setup/Repair, press Reload on the extension.',
  ...
  function errorInfo(raw){
    ...
    const known={WORKSPACE_BUSY:'errWorkspaceBusy',THREAD_BUSY:'errThreadBusy',PENDING_AGENT_CHANGES:'errPending',HOST_MAINTENANCE:'errMaintenance',WORKSPACE_REQUIRED:'errWorkspaceRequired',GEMINI_BUSY:'errBusy',PAIRING_REQUIRED:'errPairing'};
  ...
  function renderModeHelp(){const d=modeDescriptions()[mode()];if(d)modeHelp.textContent=`${d.title} · ${d.description}`;}
  ...
  function updateEmptyState(){emptyState.hidden=messages.children.length>0||Boolean(currentRun);}
  ...
  async function loadHealth(){
    try{const h=await api('/v1/health');health.textContent=t('bridgeWorking');healthDot.className='status-dot ok';versionInfo.textContent=`Host ${h.version} · Gemini CLI ${h.gemini.version||h.gemini.error||'not ready'}`;const credentialsPresent=h.auth?.credentialsPresent;authStatus.textContent=credentialsPresent===true?t('authPresent'):credentialsPresent===false?t('authMissing'):t('authUnknown');authStatus.className='subtle'+(credentialsPresent===false?' danger-text':'');}
  ...
  function addFailure(x){
    ...
    if(info.kind==='auth'){const auth=document.createElement('button');auth.type='button';auth.textContent=t('googleSignInHelp');auth.onclick=()=>$('authHelp').click();actions.append(auth);authStatus.textContent=t('authMissing');}
  ...
  async function preflightWorkspace(selectedMode=mode()){if(selectedMode==='ask')return true;const r=await api('/v1/projects');lastProjects=r.projects||lastProjects;const p=lastProjects.find(x=>x.id===project.value);if(p?.workspace)return true;runState.textContent=friendlyError('WORKSPACE_REQUIRED');await openSettings();return false;}

  async function performManagement(x,action){try{const out=await api(`/v1/runs/${x.id}/${action}`,{method:'POST',body:{}});runState.textContent=managementOutcome(action,out);await loadPending();await loadConversation();}catch(e){runState.textContent=friendlyError(e);}}
  async function loadPending(){
    if(!project.value){pendingSection.hidden=true;return;}const r=await api(`/v1/projects/${encodeURIComponent(project.value)}/pending`);pending.textContent='';const runs=r.runs||[];pendingSection.hidden=runs.length===0;const active=runs.find(x=>['QUEUED','RUNNING'].includes(x.status));
    for(const x of runs){const d=document.createElement('div');d.className='pcard';const title=document.createElement('div');title.textContent=`${modeDescriptions()[x.mode]?.title||x.mode} · ${runLabel(x.status)}`;d.append(title);if(x.error){const er=document.createElement('div');er.className='hint';er.textContent=friendlyError(x.error);d.append(er);}if(x.recoveryPid){const note=document.createElement('div');note.className='hint';note.textContent=t('recoveryPid',{pid:x.recoveryPid});d.append(note);}if(x.result?.diffSummary){const sm=document.createElement('pre');sm.className='diffsummary';sm.textContent=x.result.diffSummary;d.append(sm);}
      const row=document.createElement('div');row.className='row';if(['WAITING_APPLY','RECOVERY_REQUIRED','APPLY_CONFLICT','APPLY_RECOVERY_REQUIRED'].includes(x.status)){
        const pv=document.createElement('button');pv.textContent=t('preview');pv.onclick=async()=>{try{const out=await api(`/v1/runs/${x.id}/preview`);let pre=d.querySelector('.preview');if(!pre){pre=document.createElement('pre');pre.className='preview';d.append(pre);}pre.textContent=out.preview.items.map(i=>`### ${i.type.toUpperCase()} ${i.path} · ${i.state}\n--- BEFORE ---\n${i.before??'(missing)'}\n--- AFTER ---\n${i.after??'(deleted)'}`).join('\n\n');}catch(e){runState.textContent=friendlyError(e);}};row.append(pv);
        const action=(x.status==='WAITING_APPLY')?'apply':'reconcile';const a=document.createElement('button');a.textContent=action==='apply'?t('apply'):t('reconcile');a.onclick=()=>performManagement(x,action);row.append(a);
        if(['WAITING_APPLY','RECOVERY_REQUIRED','APPLY_CONFLICT'].includes(x.status)){const ds=document.createElement('button');ds.textContent=t('discard');ds.onclick=async()=>{if(!confirm(t('discardConfirm')))return;await performManagement(x,'discard');};row.append(ds);}
  ...
  function poll(id,assistantBox,resume=false){
    ...
    const tick=async()=>{if(pollInFlight||generation!==pollGeneration)return;pollInFlight=true;try{const r=(await api(`/v1/runs/${id}`)).run;if(generation!==pollGeneration)return;runState.textContent=runLabel(r.status);if(box)box.textContent=r.partialText||r.resultText||(r.error?friendlyError(r.error):runLabel(r.status));if(!['QUEUED','RUNNING'].includes(r.status)){clearInterval(timer);timer=null;currentRun=null;send.disabled=false;cancel.hidden=true;await loadThreads(r.threadId);await loadPending();updateEmptyState();}}catch(e){if(generation===pollGeneration)runState.textContent=friendlyError(e);}finally{pollInFlight=false;}};
  ...
  cancel.onclick=async()=>{if(currentRun)try{await api(`/v1/runs/${currentRun}/cancel`,{method:'POST',body:{}});runState.textContent=t('cancelling');}catch(e){runState.textContent=friendlyError(e);}};
  ...
  $('authHelp').onclick=()=>showHelp(t('authHelpTitle'),t('authHelpBody'));
  ...
  applyStaticI18n();loadHealth();loadProjects().catch(e=>{health.textContent=friendlyError(e);healthDot.className='status-dot bad';});setInterval(loadHealth,15000);
})();
