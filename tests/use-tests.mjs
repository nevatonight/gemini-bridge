...
test('Package: installer avoids global Gemini npm mutation and uses explicit app-local runtime repair',async()=>{
  const setup=await fsp.readFile(path.join(ROOT,'Setup.ps1'),'utf8');assert.doesNotMatch(setup,/npm\s+install\s+-g|install\s+-g\s+['\"]?@google\/gemini-cli/i);assert.match(setup,/Invoke-NativeLogged \$npm @\('install','--prefix',\$dir/);assert.match(setup,/\$GeminiVersion='0\.55\.1'/);assert.match(setup,/setup-runtime/);
});
