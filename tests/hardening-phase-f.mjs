...
test('Phase F: Google auth uses current Ask policy and sanitized Bridge environment',async()=>{
  const auth=await fsp.readFile(path.join(ROOT,'src','auth.mjs'),'utf8'),runner=await fsp.readFile(path.join(ROOT,'src','gemini.mjs'),'utf8'),ps=await fsp.readFile(path.join(ROOT,'Auth-GeminiBridge.ps1'),'utf8');
  assert.match(auth,/launchInteractiveAuth/);
  assert.match(runner,/settings\('ask'/);
  assert.match(runner,/bridgeEnv\(process\.env/);
  assert.match(runner,/shell:false/);
  assert.match(runner,/auth-empty/);
  assert.match(runner,/const keep=\['PATH'[\s\S]*'NO_PROXY'\]/);
  assert.doesNotMatch(ps,/\$env:GEMINI_CLI_HOME|\$env:GEMINI_CLI_SYSTEM_SETTINGS_PATH/);
});

test('Phase F: Setup upgrade order is manifest -> maintenance -> shutdown -> offline check -> backup -> dependency acquire -> stage -> strict health -> commit',async()=>{
  const s=await fsp.readFile(path.join(ROOT,'Setup.ps1'),'utf8'),main=s.slice(s.indexOf('$oldCfg=$null'));const marks=['Verify-ReleaseManifest $script:ReleaseRoot','Enter-MaintenanceIfRunning','Stop-AuthenticatedHost','Invoke-OfflineStateCheck','Backup-State','Ensure-Antigravity','Stage-Release','Verify-NewHost','Commit-Release','Finalize-Release'];let prev=-1;for(const m of marks){const i=main.indexOf(m);assert.ok(i>prev,`${m} order`);prev=i;}assert.match(s,/if\(-not \$script:Committed\)\{try\{Rollback-Release/);assert.match(s,/Upgrade was already committed; cleanup\/retention failure did not roll back/);
});
...
test('Phase F: release manifest verification is exhaustive and staging copies hidden files with unique nonce directories',async()=>{
  const s=await fsp.readFile(path.join(ROOT,'Setup.ps1'),'utf8');assert.match(s,/Get-FileHash -Algorithm SHA256/);assert.match(s,/Unlisted release file/);assert.match(s,/Release manifest is not exhaustive/);assert.match(s,/Get-ChildItem -LiteralPath \$script:ReleaseRoot -Force/);assert.match(s,/\.staging-'\+\$nonce/);assert.doesNotMatch(s,/\$InstallRoot\+'\.new'/);
});

test('Phase F: Antigravity CLI is minimum-gated, copied app-local, exact-version-probed, and repair uses unique sibling directories',async()=>{
  const s=await fsp.readFile(path.join(ROOT,'Setup.ps1'),'utf8');assert.match(s,/\$MinimumAntigravityVersion='1\.1\.20'/);assert.match(s,/antigravity\.google\/cli\/install\.ps1/);const probe=await fsp.readFile(path.join(ROOT,'src','managed-antigravity-probe.mjs'),'utf8');assert.match(probe,/--version/);assert.match(s,/antigravity-'\+\$sourceVersion\+'-'\+\[guid\]::NewGuid/);assert.match(s,/Copy-FileWithRetry \$externalEntry \$entry/);assert.doesNotMatch(s,/@google\/gemini-cli@|npm\s+install\s+-g/);
});
