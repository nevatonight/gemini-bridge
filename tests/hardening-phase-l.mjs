...
test('Phase L: Setup captures and logs npm/winget native output instead of leaking it into scalar return values',async()=>{
  ...
  assert.match(s,/\$npmRun=Invoke-NativeLogged \$npm/);
  assert.match(s,/\$r=Invoke-NativeLogged \$winget/);
  assert.doesNotMatch(s,/;\s*& \$npm install/);
  assert.doesNotMatch(s,/;\s*& \$winget install/);
});

test('Phase L: Setup enforces one existing path value at Node/npm/Gemini and runtime.json boundaries',async()=>{
  const s=await fsp.readFile(path.join(ROOT,'Setup.ps1'),'utf8');
  ...
  assert.match(s,/\$npm=Require-SinglePath \$npmCandidate 'npm launcher'/);
  assert.match(s,/\$entry=Require-SinglePath \(Ensure-Gemini[\s\S]*'Gemini CLI entry' \$RuntimeBase/);
  assert.match(s,/Assert-RuntimeConfigPaths \$newCfg/);
  assert.match(s,/runtime\.json geminiEntry/);
  assert.match(s,/function Find-Npm\(\[string\]\$node\)\{\$same=Join-Path \(Split-Path -Parent \$node\) 'npm\.cmd'/);
  assert.doesNotMatch(s,/Get-Command npm\.cmd/);
});
