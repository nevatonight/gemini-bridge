...
test('Phase O: Antigravity auth presence is provider-probed rather than inferred from profile files',async()=>{
  const mod=await import('../src/auth-state.mjs');const d=await tmp(),state=path.join(d,'state'),entry=path.join(d,'agy-auth');
  await fsp.writeFile(entry,`#!/usr/bin/env node\nif(process.argv.includes('--version')){console.log('1.2.3');process.exit(0)}\nif(process.argv[2]==='models'||process.argv[1]==='models')process.exit(0);\nprocess.exit(0);\n`);await fsp.chmod(entry,0o755);
  assert.equal(await mod.hasGeminiCredentials(state,{geminiEntry:entry}),true);
  assert.match(mod.geminiCredentialFile(),/Windows Credential Manager/);
});

test('Phase O: Setup shows six high-level stages, opens Dashboard, and defers sign-in to its direct button',async()=>{
  const s=await fsp.readFile(path.join(ROOT,'Setup.ps1'),'utf8');for(let i=1;i<=6;i++)assert.match(s,new RegExp(`Stage\\s+${i}\\b`));assert.match(s,/auth-status/);assert.match(s,/Launch-Dashboard\.cmd/);assert.match(s,/Sign in with Google/);const stage6=s.slice(s.indexOf("Stage 6 'Dashboard and Google sign-in readiness'"));assert.doesNotMatch(stage6,/Auth-GeminiBridge\.cmd/);
});

test('Phase O: compatibility Auth wrapper delegates to provider auth and verifies status',async()=>{
  const js=await fsp.readFile(path.join(ROOT,'src','auth.mjs'),'utf8'),ps=await fsp.readFile(path.join(ROOT,'Auth-GeminiBridge.ps1'),'utf8');assert.match(js,/launchInteractiveAuth/);assert.match(ps,/auth-status/);assert.match(ps,/GEMINI_BRIDGE_STATE/);
});
