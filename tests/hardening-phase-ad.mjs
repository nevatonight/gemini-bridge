...
test('Phase AD: health separates Bridge runtime from Google credential presence',async()=>{
  const host=await text('src/host.mjs'),ui=await text('ui/app.js');assert.match(host,/hasGeminiCredentials/);assert.match(host,/credentialsPresent/);assert.match(ui,/authStatus/);assert.match(ui,/credentialsPresent/);assert.match(ui,/Google/);
});
