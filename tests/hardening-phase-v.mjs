...
test('Setup acquires Antigravity only from the official Google installer endpoint', () => {
  assert.match(setup, /https:\/\/antigravity\.google\/cli\/install\.ps1/);
  assert.doesNotMatch(setup, /@google\/gemini-cli@|registry\.npmjs\.org/);
});
